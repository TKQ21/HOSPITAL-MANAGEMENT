...
export default function BillingPage() {
  ...
  const [form, setForm] = useState({
    patient_name: "", patient_phone: "", total_amount: "", paid_amount: "",
  ...
  const resetForm = () => {
    setForm({ patient_name: "", patient_phone: "", total_amount: "", paid_amount: "", payment_status: "pending", payment_method: "", notes: "", item_desc: "", item_amount: "" });
    setItems([]); setEditId(null); setShowForm(false);
  };
  ...
  const removeItem = (idx: number) => {
    ...
    setItems(newItems);
    setForm({ ...form, total_amount: String(newItems.reduce((s, i) => s + i.amount, 0)) });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const data: any = {
      ...
      items: items, total_amount: Number(form.total_amount) || 0,
      paid_amount: Number(form.paid_amount) || 0,
      payment_status: form.payment_status, payment_method: form.payment_method || null,
      notes: form.notes || null,
    };

    if (editId) {
      await supabase.from("billing").update(data).eq("id", editId);
      toast({ title: "Updated ✓" });
    } else {
      await supabase.from("billing").insert(data);
      toast({ title: "Created ✓" });
    }
    resetForm(); load();
  };
  ...
  const handleEdit = (b: Bill) => {
    setForm({
      ...
      total_amount: String(b.total_amount), paid_amount: String(b.paid_amount),
      payment_status: b.payment_status, payment_method: b.payment_method || "",
      notes: b.notes || "", item_desc: "", item_amount: "",
    });
  ...
  const statusColors: Record<string, string> = { pending: "neon-text-yellow", paid: "neon-text-green", partial: "neon-text-cyan", overdue: "neon-text-pink" };
    ...
    <div className="space-y-4 animate-slide-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        <div className="glass-panel rounded-xl p-3 border neon-border-green">
          ...
          <p className="font-display text-xl font-bold neon-text-green">₹{totalRevenue.toLocaleString()}</p>
        </div>
        <div className="glass-panel rounded-xl p-3 border neon-border-yellow">
          <p className="text-[10px] text-muted-foreground">Pending</p>
          <p className="font-display text-xl font-bold neon-text-yellow">₹{pendingAmount.toLocaleString()}</p>
        </div>
        <div className="glass-panel rounded-xl p-3 border neon-border-cyan">
          ...
          <p className="font-display text-xl font-bold neon-text-cyan">{bills.length}</p>
        </div>
      </div>
          ...
          <div className="border border-border/50 rounded-lg p-3 space-y-2">
            ...
            {items.map((item, idx) => (
              <div key={idx} className="flex items-center gap-2 text-xs">
                ...
                <button type="button" onClick={() => removeItem(idx)}><X className="w-3 h-3 neon-text-pink" /></button>
              </div>
            ))}
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-3">
          {filtered.map(b => (
            <div key={b.id} className="glass-panel rounded-xl p-4 border neon-border-cyan">
