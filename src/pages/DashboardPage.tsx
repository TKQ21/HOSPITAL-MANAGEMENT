...
import { Calendar, Users, Clock, CheckCircle, AlertTriangle, TrendingUp, XCircle } from "lucide-react";
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const load = () => {
      setAppointments(JSON.parse(localStorage.getItem("clinic_appointments") || "[]"));
      setPatients(JSON.parse(localStorage.getItem("clinic_patients") || "[]"));
    };
    load();
  ...
  return (
    <div className="space-y-4 sm:space-y-6 animate-slide-in">
      <div>
        ...
        <p className="text-muted-foreground text-xs sm:text-sm mt-1">Welcome back, Dr. Sharma</p>
      </div>
                ...
                <div key={appt.id} className="flex items-center justify-between p-2.5 sm:p-3 rounded-lg bg-secondary/30 hover:bg-secondary/50 transition-colors">
                  <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
                    ...
                    <div className="min-w-0">
                      <p className="text-xs sm:text-sm font-medium truncate">{appt.patientName}</p>
                      <p className="text-[10px] sm:text-xs text-muted-foreground truncate">{appt.reason || "General"} • {appt.phone}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 sm:gap-3 shrink-0 ml-2">
                    <span className="text-[10px] sm:text-xs text-muted-foreground hidden sm:block">{appt.date} {appt.time}</span>
