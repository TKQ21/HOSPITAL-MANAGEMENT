...
interface Bed {
  ...
  patient_name: string | null; patient_phone: string | null; status: string; admitted_at: string | null;
}
...
export default function BedsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const data: any = { bed_number: form.bed_number, ward_type: form.ward_type, department_id: form.department_id || null, patient_name: form.patient_name || null, patient_phone: form.patient_phone || null, status: form.status, admitted_at: form.status === "occupied" ? new Date().toISOString() : null };
    if (editId) { await supabase.from("beds").update(data).eq("id", editId); toast({ title: "Updated ✓" }); }
    else { await supabase.from("beds").insert(data); toast({ title: "Added ✓" }); }
    resetForm(); load();
  };

  const handleEdit = (b: Bed) => { setForm({ bed_number: b.bed_number, ward_type: b.ward_type, department_id: b.department_id || "", patient_name: b.patient_name || "", patient_phone: b.patient_phone || "", status: b.status }); setEditId(b.id); setShowForm(true); };
  ...
  return (
    <div className="space-y-4 animate-slide-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="glass-panel rounded-xl p-3 border neon-border-darkblue"><p className="text-[10px] text-muted-foreground">Total Beds</p><p className="font-display text-xl font-bold neon-text-darkblue">{totalBeds}</p></div>
        <div className="glass-panel rounded-xl p-3 border neon-border-green"><p className="text-[10px] text-muted-foreground">Available</p><p className="font-display text-xl font-bold neon-text-green">{totalBeds - occupied}</p></div>
        <div className="glass-panel rounded-xl p-3 border neon-border-orange"><p className="text-[10px] text-muted-foreground">Occupied</p><p className="font-display text-xl font-bold neon-text-orange">{occupied}</p></div>
        <div className={`glass-panel rounded-xl p-3 border ${icuPercent >= 85 ? "neon-border-red neon-glow-red" : "neon-border-purple"}`}>
          <p className="text-[10px] text-muted-foreground">ICU Occupancy</p>
          <div className="flex items-center gap-1">
            <p className={`font-display text-xl font-bold ${icuPercent >= 85 ? "neon-text-red" : "neon-text-purple"}`}>{icuPercent}%</p>
            {icuPercent >= 85 && <AlertTriangle className="w-4 h-4 neon-text-red animate-pulse" />}
          </div>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="glass-panel rounded-xl p-5 border neon-border-green space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <select value={form.ward_type} onChange={e => setForm({ ...form, ward_type: e.target.value })} className={inputClass}>
              <option value="General">General</option><option value="ICU">ICU</option><option value="Pediatric">Pediatric</option>
              <option value="Maternity">Maternity</option><option value="Surgery">Surgery</option><option value="Emergency">Emergency</option>
            </select>
          ...
          <button type="submit" className="flex items-center gap-2 px-5 py-2 rounded-lg bg-primary/20 border neon-border-green text-sm neon-text-green"><Save className="w-4 h-4" /> {editId ? "Update" : "Add"}</button>
        </form>
      )}
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
          {filtered.map((b, idx) => {
            const colors = ["neon-border-purple", "neon-border-orange", "neon-border-darkblue", "neon-border-red", "neon-border-pink", "neon-border-yellow"];
            const bedBorder = b.status === "occupied" ? "neon-border-orange" : b.status === "maintenance" ? "neon-border-red" : colors[idx % colors.length];
            return (
              <div key={b.id} className={`glass-panel rounded-xl p-3 border ${bedBorder}`}>
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <BedDouble className={`w-4 h-4 ${b.status === "occupied" ? "neon-text-orange" : "neon-text-green"}`} />
                    <span className="font-display text-sm font-bold">{b.bed_number}</span>
                  </div>
                  <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${b.status === "available" ? "bg-[hsl(var(--neon-green)/.15)] neon-text-green" : b.status === "occupied" ? "bg-[hsl(var(--neon-orange)/.15)] neon-text-orange" : "bg-[hsl(var(--neon-red)/.15)] neon-text-red"}`}>
                    {b.status}
                  </span>
                </div>
